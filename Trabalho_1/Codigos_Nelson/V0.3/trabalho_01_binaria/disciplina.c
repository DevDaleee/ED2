...
void InsereDadosDisc(Disciplina **raizArvDisc, int QuantBlocosCurso, int QntSemDisc) {
    NovaDisc(&(*raizArvDisc), "maneira", 100, 60, QuantBlocosCurso, QntSemDisc);
}
...
void NovaDisc(Disciplina **raizArvDisc, char *NomeDisc, int Bloco, int CargaHoraria, int QuantBlocosCurso, int QntSemDisc) {
    ...
    if ((VerificaBlocoDisc(QuantBlocosCurso, Bloco) == 0) || (VerificaCH(QntSemDisc, CargaHoraria) == 0)) {
       printf("ERRO ao inserir disciplina - %s\n", NomeDisc);
    }
    else {
...
Disciplina* MaiorDiscEsq(Disciplina *raizArvDisc) {
    if (raizArvDisc->dir != NULL){
        return MaiorDiscEsq(raizArvDisc->dir);
    }
    else {
        return (raizArvDisc);
    }
...
void RemoveDisc(Disciplina **raizArvDisc, int CodDisc, char *situacao) {
    if ((*raizArvDisc) != NULL) {
        ...
        if ((*raizArvDisc)->cod_disc == CodDisc) {
            if (DiscFolha(*raizArvDisc)) {
                Aux = (*raizArvDisc);
                (*raizArvDisc) = NULL;
            }
            else if (DiscDoisFilhos(*raizArvDisc)) {
                Filho = MaiorDiscEsq(*raizArvDisc);
                Aux = (*raizArvDisc);
